...
with source_data as (
  ...
  select *
  from read_json_auto(
    's3://sandbox-datalake-bronze/davlab/notion/{{ notion_date }}/points.json'
  )
...
exploded as (
  ...
  select
    json_extract(results_elem, '$.id')::VARCHAR as point_id,
    ...
    json_extract(results_elem, '$.properties.Points.number')::int as points,
